...
for node in $CLUSTER; do
  ...
  ssh $node "rm -rf /home/auroflow/code/vector-search/rocksdb-stream/tmp/*"
done
...
ssh auroflow@$MASTER -i /home/auroflow/.ssh/id_rsa 'flink run -c cn.edu.zju.daily.HnswLibSearchJob --jobmanager=10.214.151.23:8081 /home/auroflow/code/vector-search/rocksdb-stream/target/rocksdb-stream-1.0-SNAPSHOT.jar'
echo "done"
