...
# Get current time
DATE=$(date +"%m%d-%H%M")
syslog_dir="$SYSLOG_DIR/$DATE-qdrant"
...
for node in $MONITORED; do
  ...
  ssh $SSH_CONFIG root@$node 'nohup bash -c "/usr/local/bin/pidstat 2 -urd -p $(pgrep -f "java|qdrant|docker|container" | tr "\n" ",") > '$syslog_dir'/pidstat.log 2>&1 &" </dev/null >/dev/null 2>/dev/null'
...
ssh $SSH_CONFIG $RUN_HOST 'nohup flink run -c cn.edu.zju.daily.QdrantSeparatedStreamSearchJob --jobmanager='$FLINK_MASTER':'$FLINK_MASTER_PORT' '$FLINK_FRONTEND_JAR' '$FLINK_FRONTEND_DIR'/src/main/resources/params.yaml > '$FLINK_FRONTEND_DIR'/nohup.out &'
