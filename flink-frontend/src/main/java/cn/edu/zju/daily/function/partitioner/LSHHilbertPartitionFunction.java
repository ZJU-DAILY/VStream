...
/**
 * <p>使用 LSH 函数和 Hilbert 曲线为向量数据和查询分配分区. 该函数扩展 RichCoFlatMapFunction，需要同时处理输入向量和查询向量。</p>
 *
 ...
 * 个分区 ID。</p>
 */
public class LSHHilbertPartitionFunction extends RichCoFlatMapFunction<FloatVector, FloatVector, PartitionedData> implements PartitionFunction {
    ...
    public LSHHilbertPartitionFunction(Random random,
                                       int dim,
                                       int numHashFamilies,
                                       ...
                                       long maxTTL,
                                       int numPartitions) {
        this(random, dim, numHashFamilies, numHashFunctions, hashWidth, numHilbertBits, updateInterval, maxRetainedElements, maxTTL, 0, numPartitions);
    }
...
//        }
        ...
        for (int i = 0; i < numHashFamilies; i++) {
            partitioners.add(new L2HilbertPartitioner(dim, numHashFunctions, hashWidth, numHilbertBits, updateInterval,
                    maxRetainedElements, maxTTL, numPartitions, new Random(random.nextLong())));
        }
    ...
    public void open(Configuration parameters) throws Exception {
        ...
        fakeTsNano = 0;
        LOG.info("LSHHilbertPartitionFunction initialized with insert interval {} ns.", fakeInsertIntervalNano);
    ...
    public void flatMap2(FloatVector value, Collector<PartitionedData> out) throws Exception {
        ...
        for (int partition : partitions) {
            out.collect(new PartitionedQuery(nodeIdToKeyMap.get(partition), numPartitionsSent, value));
        }
    }
}
