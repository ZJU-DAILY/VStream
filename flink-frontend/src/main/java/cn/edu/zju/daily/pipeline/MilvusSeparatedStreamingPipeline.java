...
public class MilvusSeparatedStreamingPipeline {
    ...
    public SingleOutputStreamOperator<SearchResult> apply(DataStream<FloatVector> data, DataStream<FloatVector> query) {

        // This implementation is partly wrong, because it uses different partitioners for data and query, which are
        // "trained" with different data, resulting in different partitioning schemes. However, this prevents
        SingleOutputStreamOperator<PartitionedData> partitionedData = data.flatMap(getUnaryPartitioner(false))
                .name("data partition")
                .setParallelism(1)
                .setMaxParallelism(1);
        ...
        SingleOutputStreamOperator<PartitionedData> partitionedQuery = query.flatMap(getUnaryPartitioner(true))
    ...
    @Deprecated
    public SingleOutputStreamOperator<SearchResult> applyWithJoinedPartitioner(DataStream<FloatVector> data, DataStream<FloatVector> query) {
        PartitionFunction partitioner = getPartitioner();
        OutputTag<PartitionedData> partitionedQueryTag = new OutputTag<PartitionedData>("partitioned-query") {
        };

        SingleOutputStreamOperator<PartitionedData> partitionedData = data.connect(query).flatMap(partitioner)
                .process(new PartitionedDataSplitFunction(partitionedQueryTag))
                .setParallelism(1)
                .setMaxParallelism(1)
                .name("partition");

        SideOutputDataStream<PartitionedData> partitionedQuery = partitionedData.getSideOutput(partitionedQueryTag);
    ...
    private SingleOutputStreamOperator<SearchResult> applyToQueryStream(DataStream<PartitionedData> data) {
        SingleOutputStreamOperator<SearchResult> rawResults = data.keyBy(PartitionedData::getPartitionId)
                .process(new MilvusKeyedQueryProcessFunction(params))
                .setParallelism(params.getParallelism())
                .setMaxParallelism(params.getParallelism())
                .name("query process")
                .slotSharingGroup("process");
        return rawResults.keyBy(SearchResult::getQueryId)
