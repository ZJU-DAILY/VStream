...
import cn.edu.zju.daily.partitioner.curve.HilbertCurve.Builder;
...
public class VStreamRecallTestJob {
    private static final Parameters params = new Parameters();

    private static final int total = 1428000;
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int j = 0; j < numSamples; j++) {
            queries.add(vectors.get(random.nextInt(vectors.size())));
        }

        for (int replication = 14; replication < 15; replication++) {
            test(vectors, queries, replication, random);
        }
    }

    private static void test(
            List<VectorData> vectors, List<VectorData> queries, int replication, Random random)
            throws Exception {
        ...
        insert(vectors, indexes, partitioner);
