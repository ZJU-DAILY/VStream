...
public class StreamSearchLocalAccuracyTest {
    ...
    private static final Parameters params = Parameters.load(
            "/home/auroflow/code/vector-search/rocksdb-stream/src/main/resources/params-local.yaml", false);
    ...
    public static void main(String[] args) throws Exception {
        ...
        GroundTruthResultIterator gtIt = GroundTruthResultIterator.fromFile(params.getGroundTruthPath(), params.getK());
        while (gtIt.hasNext()) {
        ...
        while (searchResults.hasNext()) {
            ...
            if (accuracies.size() % 100 == 99) {
                System.out.println((accuracies.size() + 1) + " queries: " + accuracies.stream().skip(Math.max(0, accuracies.size() - 99)).collect(toList()));
                // average
                System.out.println("average: " + accuracies.stream().mapToDouble(Float::doubleValue).average().getAsDouble()
                        + ", latency: " + latencies.stream().mapToLong(Long::longValue).average().getAsDouble());
            }
        }
