...
public class DataUploader {
    ...
    public void convert() throws IOException {
        ...
        while (it.hasNext()) {
            hadoopFileHelper.writeString(
                    outputStreamBufferedWriterTuple2.getRight(), parser.unparse(it.next()));
            if (i % 500_000 == 0) {
    ...
    public static void main(String[] args) {
        if (args.length == 1 && args[0].equals("-h")) {
            ...
            System.out.println();
            System.out.println(
                    "Usage: DataUploader <sourcePath> <hdfsAddress> <hdfsUser> <hdfsPath> [<loop> <limit>]");
            System.out.println();
        ...
        try {
            DataUploader uploader =
                    new DataUploader(sourcePath, hdfsAddress, hdfsUser, hdfsPath, loop, limit);
            uploader.convert();
        } catch (IOException e) {
            e.printStackTrace();
        }
