...
/**
 * <p>Main insert & search process function.</p>
 *
 * <p>为插入和搜索分别创建一个线程池，</p>
 */
public class RocksDBKeyedProcessFunction
    ...
    private final static Logger logger = LoggerFactory.getLogger(RocksDBKeyedProcessFunction.class);
    ...
    private void insert(FloatVector vector, int nodeId) throws Exception {
        ...
        if (vector.getId() % 16043 == 0) {  // gcd(1z
            logger.info("Partition {} (ts = {}): Inserting vector #{}", nodeId, vector.getEventTime(), vector.getId());
        }
        byte[] id = new byte[8];
    ...
    public void search(StreamRecord<PartitionedData> query, int nodeId, int numPartitionsSent,
        ...
        logger.info("Partition {} ({} ms since partition): Searching query #{} in {} ms", query.getValue().getPartitionId(),
                (current - partitionedAt), queryVector.getId(), (current - start));

        SearchResult r = deserializeSearchResult(resultBytes, nodeId, queryVector.getId(), numPartitionsSent, queryVector.getEventTime());
    ...
    public void processElement(
        ...
        StreamRecord<PartitionedData> record = (currentTimestamp != null) ? new StreamRecord<>(data, currentTimestamp) : new StreamRecord<>(data);
