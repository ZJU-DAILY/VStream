...
import cn.edu.zju.daily.function.partitioner.sax.HotTracer;
import cn.edu.zju.daily.function.partitioner.sax.SAX;
import java.util.*;
...
public class OdysseyPartitionFunction extends RichPartitionFunction {
    ...
    public void flatMap1(VectorData data, Collector<PartitionedElement> out) throws Exception {
        ...
        if (hotSAXWords.contains(encode)) {
            ...
            groupId = encode % replicationGroups.size();
            if (isSkewed(groupId)) {
                groupId = random.nextInt(replicationGroups.size());
            }
        }
        ...
        for (int partition : partitions) {
            out.collect(new PartitionedData(partition, data));
        }
    ...
    private static List<List<Integer>> createReplicationGroups(
