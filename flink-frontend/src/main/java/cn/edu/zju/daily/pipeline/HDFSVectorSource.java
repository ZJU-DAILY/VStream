...
public class HDFSVectorSource {
    ...
    public SingleOutputStreamOperator<FloatVector> getSourceStream(boolean throttle) {
        if (throttle) {
            return get(params.getHdfsAddress(), params.getSourcePath(), params.getInsertThrottleThresholds(), params.getInsertRates(), "source", 5);
        } else {
            return get(params.getHdfsAddress(), params.getSourcePath(), Collections.singletonList(0L), Collections.singletonList(0L), "source", 5);
        }
    }

    public SingleOutputStreamOperator<FloatVector> getQueryStream(boolean throttle) {
        if (throttle) {
            return get(params.getHdfsAddress(), params.getQueryPath(), params.getQueryThrottleThresholds(), params.getQueryRates(), "query", 1);
        } else {
            return get(params.getHdfsAddress(), params.getQueryPath(), Collections.singletonList(0L), Collections.singletonList(0L), "query", 1);
        }
    }
     ...
     * <p>
     * ONLY FOR TESTING -- CANNOT HANDLE LARGE DATA.
     *
     * @return
     * @throws Exception
     */
    public SingleOutputStreamOperator<PartitionedData> getPartitionedSourceAndQueryStream() throws Exception {
        // Collect source and query data to master. For now we only support 1M source and 1M query.
        List<FloatVector> source = getSourceStream(false).executeAndCollect("fetch source", 1_000_000);
        List<FloatVector> queries = getQueryStream(false).executeAndCollect("fetch queries", 1_000_000);
    ...
    private SingleOutputStreamOperator<FloatVector> get(String hdfsAddress, String hdfsPath, List<Long> thresholds, List<Long> rates, String name, int sourceParallelism) {
        FileSource<String> fileSource = FileSource.forRecordStreamFormat(
                new TextLineInputFormat(),
                new Path(hdfsAddress + hdfsPath)
        ).build();
        FloatVectorRateLimiter limiter = new FloatVectorRateLimiter(thresholds, ratesToIntervals(rates));
        return env.fromSource(fileSource, WatermarkStrategy.noWatermarks(), "hdfs-vector-source")
                .name(name + " input")
                ...
                .name(name + " throttle")
                .returns(FloatVector.class)
                .assignTimestampsAndWatermarks(WatermarkStrategy.<FloatVector>forMonotonousTimestamps()
                        .withTimestampAssigner((vector, timestamp) -> {
                            long current = System.currentTimeMillis();
                            vector.setEventTime(current);
                            return current;  // currently, use system time as event time
                        }))
