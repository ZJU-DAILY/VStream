...
public final class RocksDBResourceContainer implements AutoCloseable {
    ...
    /** Gets the RocksDB {@link ColumnFamilyOptions} to be used for all RocksDB instances. */
    public VectorColumnFamilyOptions getVectorColumnOptions() {
        // initial options from common profile
        VectorColumnFamilyOptions opt = createBaseCommonVectorColumnOptions();
        handlesToClose.add(opt);

        // load configurable options on top of pre-defined profile
        setVectorColumnFamilyOptionsFromConfigurableOptions(opt, handlesToClose);

        // add user-defined options, if specified
        if (optionsFactory != null) {
            opt = optionsFactory.createVectorColumnOptions(opt, handlesToClose);
        }
    ...
    public VectorSearchOptions getVectorSearchOptions() {
        VectorSearchOptions opt = new VectorSearchOptions();
        handlesToClose.add(opt);

        setVectorSearchOptionsFromConfigurableOptions(opt, handlesToClose);

        // add user-defined options factory, if specified
        if (optionsFactory != null) {
            opt = optionsFactory.createVectorSearchOptions(opt, handlesToClose);
        }
    ...
    /** Create a {@link VectorColumnFamilyOptions} for RocksDB, including some common settings. */
    VectorColumnFamilyOptions createBaseCommonVectorColumnOptions() {
        return new VectorColumnFamilyOptions();
    }
