...
class MemTableRep {
 public:
  ...
  virtual void Search(const LookupKey& key, std::string* value, Status* s,
  ...
  // Return the vector index name of memtablerep
  // Default: nullptr that means the type isn't the vector index memtable
  virtual const char* VectorIndexName() const { return nullptr; }

  // Return true if the current MemTableRep is HnswMemtableRep and cur_size >=
  // max_size
  virtual bool ShouldFlushNow() const { return false; }

  // Return the vector index of memtablerep
  // Default: nullptr that means the type isn't the vector index memtable
  // Currently, only hnsw index is supported
  using dist_t = float;
  virtual std::shared_ptr<hnswlib::HierarchicalNSW<dist_t>> GetVectorIndex()
      const;
