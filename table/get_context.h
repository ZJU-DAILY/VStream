...
struct GetContextStats {
  ...
  //  uint64_t num_cache_vector_index_add = 0;
  //  uint64_t num_cache_vector_index_add_redundant = 0;
  //  uint64_t num_cache_vector_index_bytes_insert = 0;
  ...
  //  uint64_t num_vector_index_read = 0;
...
class GetContext {
 public:
  ...
  // Only for vector search that does not set state to kFound
  void SaveValue(const Slice& value);

  PinnableSlice* Value() { return pinnable_val_; }

  void SetFileNumber(const uint64_t file_number) { file_number_ = file_number; }
