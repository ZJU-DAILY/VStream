...
#include "plugin/vectorbackend/table/hnsw_block.h"
...
Block::NewGorillaCompressedDataBlockIterator(
    VECTORBACKEND_NAMESPACE::GorillaCompressedDataBlockIter* iter,
    const vectorbackend::HnswTableOptions* hnsw_options,
    const std::string& table_name, bool block_contents_pinned) {
  VECTORBACKEND_NAMESPACE::GorillaCompressedDataBlockIter* ret_iter;
  if (iter != nullptr) {
    ret_iter = iter;
  } else {
    ret_iter = new VECTORBACKEND_NAMESPACE::GorillaCompressedDataBlockIter;
  }
  if (size_ < 2 * sizeof(uint32_t)) {
    ret_iter->Invalidate(Status::Corruption("bad block contents"));
    return ret_iter;
  ...
  ret_iter->Initialize(data_, restart_offset_, num_restarts_,
                       read_amp_bitmap_.get(), block_contents_pinned,
                       hnsw_options->dim, hnsw_options->M,
                       hnsw_options->block_restart_interval, table_name,
                       hnsw_options->block_cache);

  block_restart_interval_ = ret_iter->block_restart_interval_;
  return ret_iter;
}

VECTORBACKEND_NAMESPACE::HnswIndexBlockIter* Block::NewHnswIndexBlockIterator(
    VECTORBACKEND_NAMESPACE::HnswIndexBlockIter* iter,
    bool block_contents_pinned) {
  VECTORBACKEND_NAMESPACE::HnswIndexBlockIter* ret_iter;
  if (iter != nullptr) {
    ret_iter = iter;
  } else {
    ret_iter = new VECTORBACKEND_NAMESPACE::HnswIndexBlockIter;
  }
  if (size_ < 2 * sizeof(uint32_t)) {
    ret_iter->Invalidate(Status::Corruption("bad block contents"));
    return ret_iter;
  ...
  ret_iter->Initialize(data_, restart_offset_, block_contents_pinned);
