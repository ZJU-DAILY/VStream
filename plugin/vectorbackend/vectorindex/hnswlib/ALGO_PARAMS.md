...
An example of tuning the parameters can be found in [TESTING_RECALL.md](../hnswlib_/TESTING_RECALL.md)
...
### data_level0_memory_
$$
Usage = cur_element_count * size_data_per_element_ = cur_element_count * (8 + M * 4 + dim * 4 + 8 + 8)
$$
### linkLists_
E(level) = 1 / log(1.0 * M)
$$
Usage = cur_element_count * (size_links_per_element_ * level) \\
= cur_element_count_ * ((maxM_ * sizeof(tableint) + sizeof(linklistsizeint)) * 1 / log(1.0 * M)) \\
= cur_element_count * ((M * 4 + 4) / log(1.0 * M))
$$
### metadata
$$
Usage = 11 * sizeof(size_t) + sizeof(unsigned int) + sizeof(double) = 11 * 8 + 4 + 8 = 104
$$
### Total
$$
Usage = cur_element_count * (8 + M * 4 + dim * 4 + 8 + 8) + cur_element_count * ((M * 4 + 4) / log(1.0 * M)) + 104 \\
= cur_element_count * (8 + M * 4 + dim * 4 + 8 + 8 + ((M * 4 + 4) / log(1.0 * M))) + 104 \\
= cur_element_count * (24 + M * 4 + dim * 4 + ((M * 4 + 4) / log(1.0 * M))) + 104 \\
