...
// The file contains some member functions of HnswTable that
// cannot be implemented in hnsw_table_reader.cc because
// it's called by other files (e.g. hnsw_table_reader.h) and
// are templates.
...
template <>
struct IterTraits<GorillaCompressedDataBlockIter> {
...
HnswTable::InitBlockIterator<GorillaCompressedDataBlockIter>(
    const HnswTable::Rep* rep, Block* block, BlockType block_type,
...
template <typename TBlockIter>
TBlockIter* HnswTable::NewDataBlockIterator(
    const ReadOptions& ro, const BlockHandle& handle, TBlockIter* input_iter,
    BlockType block_type, FilePrefetchBuffer* prefetch_buffer,
    bool for_compaction, bool async_read, Status& s) const {
  ...
  if (rep_->uncompression_dict_reader && block_type == BlockType::kData) {
    ...
    s = RetrieveBlock(prefetch_buffer, ro, handle, dict,
                      &block.As<IterBlocklike>(), nullptr /* get_context */,
                      nullptr /* lookup_context */, for_compaction,
                      /* use_cache */ true, async_read);
  } else {
    s = RetrieveBlock(prefetch_buffer, ro, handle,
                      UncompressionDict::GetEmptyDict(),
                      &block.As<IterBlocklike>(), nullptr /* get_context */,
                      nullptr /* lookup_context */, for_compaction,
                      /* use_cache */ true, async_read);
...
// into an iterator over the contents of the corresponding block.
// If input_iter is null, new an iterator
// If input_iter is not null, update this iter and return it
template <typename TBlockIter>
TBlockIter* HnswTable::NewDataBlockIterator(const ReadOptions& ro,
                                            CachableEntry<Block>& block,
