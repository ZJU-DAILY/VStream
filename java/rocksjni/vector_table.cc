...
#include "include/org_rocksdb_HnswTableOptions.h"
...
JNIEXPORT jlong JNICALL Java_org_rocksdb_HnswTableOptions_newTableFactoryHandle(
    ...
    jint jblock_cache_num_shard_bits, jlong jdim, jbyte jspace, jlong jm) {
  ROCKSDB_NAMESPACE::VECTORBACKEND_NAMESPACE::HnswTableOptions options;
  ...
  options.index_shortening =
      ROCKSDB_NAMESPACE::IndexShorteningModeJni::toCppIndexShorteningMode(
          jindex_shortening);
  options.dim = static_cast<size_t>(jdim);
  switch (jspace) {
    case 0x0:
      options.space = hnswlib::SpaceType::L2;
      break;
    case 0x1:
      options.space = hnswlib::SpaceType::IP;
      break;
    default:
      options.space = hnswlib::SpaceType::L2;
  }
  options.M = static_cast<size_t>(jm);

  return GET_CPLUSPLUS_POINTER(
      ROCKSDB_NAMESPACE::VECTORBACKEND_NAMESPACE::NewHnswTableFactory(options));
}
