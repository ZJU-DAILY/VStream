...
void FlushJob::PickMemTable() {
  ...
  if (m->VectorIndexName()) {
    meta_.vector_sst_file_metrics =
        new VECTORBACKEND_NAMESPACE::VectorSSTFileMetrics();
  }
...
Status FlushJob::WriteLevel0Table() {
...

  {
    ...
    for (MemTable* m : mems_) {
      ...
      latest_timestamp = std::min(m->GetLatestTimestamp(), latest_timestamp);
      memtables.push_back(m->NewIterator(ro, &arena));
      auto* range_del_iter = m->NewRangeTombstoneIterator(
          ro, kMaxSequenceNumber, true /* immutable_memtable */);
      if (range_del_iter != nullptr) {
    ...
    meta_.lastest_key_time = latest_timestamp;
...

    {
      ...
      TableBuilderOptions tboptions(
          *cfd_->ioptions(), mutable_cf_options_, cfd_->internal_comparator(),
          ...
          meta_.fd.GetNumber());
      // Bring hnsw to TableBuilderOptions so that TableBuilder can use it.
      if (mems_.size() == 1) {
        const char* vector_index_name = mems_.front()->VectorIndexName();
        if (vector_index_name != nullptr &&
            strcmp(mems_.front()->VectorIndexName(), "hnsw") != 0) {
          tboptions.hnsw = mems_.front()->GetVectorIndex();
        }
      }
  ...
  if (s.ok() && has_output) {
    ...
    edit_->AddFile(0 /* level */, meta_.fd.GetNumber(), meta_.fd.GetPathId(),
                   ...
                   meta_.tail_size, meta_.lastest_key_time,
                   meta_.vector_sst_file_metrics);
