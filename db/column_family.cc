...
Status ColumnFamilyHandleImpl::GetDescriptor(
    VECTORBACKEND_NAMESPACE::VectorCFDescriptor* desc) {
  // accessing mutable cf-options requires db mutex.
  // may cause deadlock.
  // InstrumentedMutexLock l(mutex_);
  VECTORBACKEND_NAMESPACE::VectorOptions vector_options;
  db_->GetVectorOptions(this, &vector_options);
  VECTORBACKEND_NAMESPACE::VectorColumnFamilyOptions vcf_options{
      vector_options};
  *desc = VECTORBACKEND_NAMESPACE::VectorCFDescriptor(cfd()->GetName(),
                                                      vcf_options);
  return Status::OK();
}
