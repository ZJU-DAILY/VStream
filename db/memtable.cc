...
bool MemTable::ShouldFlushNow() {
  if (UNLIKELY(table_->ShouldFlushNow())) {
    return true;
  }
...
using dist_t = float;
std::shared_ptr<hnswlib::HierarchicalNSW<dist_t>> MemTable::GetVectorIndex()
    const {
  return table_->GetVectorIndex();
}
...
Status MemTable::Add(SequenceNumber s, ValueType type,
  ...
  if (table->VectorIndexName() != nullptr) {
    ...
    if (LIKELY(ts > latest_key_time)) {
      latest_key_time_.compare_exchange_strong(latest_key_time, ts,
                                               std::memory_order_relaxed,
                                               std::memory_order_relaxed);
    }
  }
...
bool MemTable::Get(const LookupKey& key, std::string* value,
  ...
  if (table_->VectorIndexName() != nullptr) {
    uint64_t ts = static_cast<const VECTORBACKEND_NAMESPACE::VectorSearchOptions*>(&read_opts)->ts;
    uint64_t latest_key_time = latest_key_time_.load(std::memory_order_relaxed);
    if (ts > latest_key_time) {
      return false;
    }
    table_->Search(key, value, s, read_opts, immutable_memtable);
    return false;
  }
  ...
  if (bloom_filter_ && !may_contain) {
    ...
    *seq = kMaxSequenceNumber;
    ...
    GetFromTable(key, *max_covering_tombstone_seq, do_merge, callback,
                 ...
                 seq, &found_final_value, &merge_in_progress, &read_opts);
  }
...
void MemTable::MultiGet(const ReadOptions& read_options, MultiGetRange* range,
  ...
  for (auto iter = temp_range.begin(); iter != temp_range.end(); ++iter) {
    ...
    GetFromTable(*(iter->lkey), iter->max_covering_tombstone_seq, true,
                 ...
                 &found_final_value, &merge_in_progress, &read_options);
