...
namespace VECTORBACKEND_NAMESPACE {
...
struct FileMetaData {
  ...
  VECTORBACKEND_NAMESPACE::VectorSSTFileMetrics* vector_sst_file_metrics =
      nullptr;  // Used for vectorbackend only to collect several metrics.
  ...
  // For vector search in the time window
  uint64_t lastest_key_time = std::numeric_limits<uint64_t>::max();
  ...
  FileMetaData(
      ...
      VECTORBACKEND_NAMESPACE::VectorSSTFileMetrics* _vector_sst_file_metrics =
          nullptr,
      uint64_t _latest_key_time = std::numeric_limits<uint64_t>::max());
...
class VersionEdit {
 public:
  ...
  void AddFile(
      ...
      uint64_t latest_key_time = std::numeric_limits<uint64_t>::max(),
      VECTORBACKEND_NAMESPACE::VectorSSTFileMetrics* vector_sst_file_metrics =
          nullptr);
