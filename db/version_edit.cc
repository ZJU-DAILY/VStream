...
FileMetaData::FileMetaData(
    ...
    const uint64_t _compensated_range_deletion_size, uint64_t _tail_size,
    vectorbackend::VectorSSTFileMetrics* _vector_sst_file_metrics,
    uint64_t _latest_key_time)
    : fd(file, file_path_id, file_size, smallest_seq, largest_seq),
      ...
      vector_sst_file_metrics(_vector_sst_file_metrics),
      lastest_key_time(_latest_key_time) {
...
void VersionEdit::AddFile(
    int level, uint64_t file, uint32_t file_path_id, uint64_t file_size,
    const InternalKey& smallest, const InternalKey& largest,
    const SequenceNumber& smallest_seqno, const SequenceNumber& largest_seqno,
    bool marked_for_compaction, Temperature temperature,
    uint64_t oldest_blob_file_number, uint64_t oldest_ancester_time,
    uint64_t file_creation_time, uint64_t epoch_number,
    const std::string& file_checksum,
    const std::string& file_checksum_func_name, const UniqueId64x2& unique_id,
    const uint64_t compensated_range_deletion_size, uint64_t tail_size,
    uint64_t latest_key_time,
    vectorbackend::VectorSSTFileMetrics* vector_sst_file_metrics) {
  ...
  new_files_.emplace_back(
      level, FileMetaData(file, file_path_id, file_size, smallest, largest,
                          ...
                          epoch_number, file_checksum, file_checksum_func_name,
                          unique_id, compensated_range_deletion_size, tail_size,
                          vector_sst_file_metrics, latest_key_time));
...
bool VersionEdit::EncodeTo(std::string* dst) const {
  ...
  for (size_t i = 0; i < new_files_.size(); i++) {
    ...
    if (f.vector_sst_file_metrics) {
      PutVarint32(dst, NewFileCustomTag::kVectorSSTMetrics);
      std::string encoded = f.vector_sst_file_metrics->Encode();
    ...
    while (true) {
      ...
      switch (custom_tag) {
        ...
        case kTemperature:
          ...
          break;
        case kVectorSSTMetrics: {
          VECTORBACKEND_NAMESPACE::VectorSSTFileMetrics vector_sst_file_metrics;
          if (!vector_sst_file_metrics.Decode(field)) {
            return "invalid vector sst file metrics";
          }
          uint64_t expected_zero = 0;
          std::chrono::milliseconds ts =
              std::chrono::duration_cast<std::chrono::milliseconds>(
                  std::chrono::system_clock::now().time_since_epoch());
          auto ts_t = static_cast<uint64_t>(ts.count());
          vector_sst_file_metrics.last_timestamp.compare_exchange_strong(
              expected_zero, ts_t, std::memory_order_relaxed);
          f.vector_sst_file_metrics = &vector_sst_file_metrics;
          break;
        }
        case kUniqueId:
