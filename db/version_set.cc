...
void Version::Get(const ReadOptions& read_options, const LookupKey& k,
  ...
  switch (read_options.GetType()) {
    case ReadOptions::kDefault: {
        ...
        *status = table_cache_->Get(
        ...
        switch (get_context.State()) {
          ...
          case GetContext::kMerge:
            // TODO: update per-level perfcontext user_key_return_count for
            // kMerge
            break;
          case GetContext::kFound:
            ...
            if (is_blob_index && do_merge && (value || columns)) {
              assert(!columns || (!columns->columns().empty() &&
                                  columns->columns().front().name() ==
    ...
    case ReadOptions::kVectorSearch: {
      auto* vector_search_options =
          static_cast<const VECTORBACKEND_NAMESPACE::VectorSearchOptions*>(
              &read_options);
      if (!vector_search_options->search_sst) {
        break;
      }
      if (vector_search_options->trigger_sort) {
        std::unique_lock _lock =
            std::unique_lock(storage_info_.level_files_lock_);
        int64_t _current_time = 0;
        *status = clock_->GetCurrentTime(&_current_time);
        ...
        for (int i = 0; i < storage_info_.num_non_empty_levels_; ++i) {
          {
            FdWithKeyRange* p_level_files =
                storage_info_.level_files_brief_[i].files;
            auto* p_level_files_brief = &(storage_info_.level_files_brief_[i]);
            // Update file's temperature
            for (size_t j = 0; j < p_level_files_brief->num_files; ++j) {
              auto* metrics =
                  p_level_files[j].file_metadata->vector_sst_file_metrics;
              if (metrics != nullptr) {
                metrics->UpdateTemperature(
                    p_level_files[j].file_metadata->file_creation_time,
                    current_time);
              }
#ifdef TRACER
              ROCKS_LOG_HEADER(
                  info_log_,
                  "File %" PRIu64
                  ": access score %f, hit score %f, sim score %f, age score "
                  "%f, temperature %f",
                  p_level_files[j].file_metadata->fd.GetNumber(),
                  metrics->access_score.load(), metrics->hit_score.load(),
                  metrics->sim_score.load(), metrics->get_age_score(),
                  metrics->temperature.load());
#endif
            }

            // Sort files by temperature
            std::sort(p_level_files,
                      p_level_files + p_level_files_brief->num_files,
                      [](const FdWithKeyRange& fd1, const FdWithKeyRange& fd2) {
                        return fd1.file_metadata->vector_sst_file_metrics
                                   ->temperature.load() >
                               fd2.file_metadata->vector_sst_file_metrics
                                   ->temperature.load();
                      });
          }
        }
      ...
      VECTORBACKEND_NAMESPACE::VectorSearchFilePicker fp(
          &storage_info_.level_files_brief_,
          storage_info_.num_non_empty_levels_);

      const VECTORBACKEND_NAMESPACE::VectorColumnFamilyInfo* vcf_info = nullptr;
      {
        MutexLock l(vset_->vcf_mutex_);
        auto info_it = vset_->vcf_info_->find(cfd_->GetID());
        if (info_it != vset_->vcf_info_->end()) {
          vcf_info = &(info_it->second);
        } else {
          *status = Status::InvalidArgument("Vector column family not found",
                                            cfd_->GetName());
          return;
        }
      }
      auto& mutable_vcf_options = vcf_info->mutable_vcf_options;
      float cur_sim = 0.0f;

      size_t searched_sst_num = 0;
      auto num_files = fp.GetNumFilesInCurrentLevel();
      auto f = fp.GetNextFile();
      std::vector<ObsoleteFileInfo> sst_delete_candidates;
      static std::vector<ObsoleteBlobFileInfo> blob_delete_candidates;  // dummy
      std::string time_log;
      while (f != nullptr) {
        ...
        ++searched_sst_num;  // before or after continue???
        if (get_context.sample()) {
        ...
        *status = table_cache_->Get(
            ...
            mutable_cf_options_.block_protection_bytes_per_key,
            mutable_cf_options_.prefix_extractor,
            cfd_->internal_stats()->GetFileReadHist(fp.GetHitFileLevel()), true,
            fp.GetHitFileLevel(), max_file_size_for_l0_meta_pin_);
        // TODO: examine the behavior for corrupted key
        if (timer_enabled) {
          PERF_COUNTER_BY_LEVEL_ADD(get_from_table_nanos, timer.ElapsedNanos(),
        ...
        auto file_location = storage_info_.file_locations_[f->fd.GetNumber()];
        auto meta_data =
            storage_info_
                .files_[file_location.GetLevel()][file_location.GetPosition()];

        auto* _vector_sst_file_metrics = f->vector_sst_file_metrics;
        _vector_sst_file_metrics->access_count++;
        uint32_t total_dist = DecodeFixed32(status->getState());
        _vector_sst_file_metrics->sim_count +=
            *reinterpret_cast<const float*>(&total_dist);
        // 使用 last item 的 dist 作为 early termination 判断的条件
        const char* data = value->data();
        uint16_t size = DecodeFixed16(data);
        data += sizeof(uint16_t) +
                (size - 1) * (sizeof(float) + sizeof(unsigned long) +
                              sizeof(SequenceNumber) + sizeof(uint64_t));
        cur_sim = *reinterpret_cast<const float*>(data);
        if (searched_sst_num >
                mutable_vcf_options.termination_lower_bound * num_files &&
            cur_sim <= vector_search_options->termination_factor *
                           mutable_vcf_options.termination_threshold.load(
      ...
      if (LIKELY(searched_sst_num > 0)) {
        // update hit count
        // skip result size, first (dist, label, seq)
        const char* data = value->data();
        uint16_t size = DecodeFixed16(data);
        data += sizeof(uint16_t);
        // get file number
        uint64_t file_number = DecodeFixed64(data);
        std::unordered_map<uint64_t, uint16_t> map;
        for (size_t i = 0; i < size - 1; ++i) {
          data += sizeof(float) + sizeof(uint64_t) + sizeof(SequenceNumber);
          uint64_t file_number = DecodeFixed64(data);
          auto it = map.find(file_number);
          if (it == map.end()) {
            map[file_number] = 1;
          } else {
            it->second++;
        ...
        auto f = fp.GetNextFile();
        while (f != nullptr) {
          auto it = map.find(f->fd.GetNumber());
          if (it != map.end()) {
            auto* _vector_sst_file_metrics = f->vector_sst_file_metrics;
            _vector_sst_file_metrics->hit_count += it->second;
          }
          f = fp.GetNextFile();
        }

        auto& termination_threshold = const_cast<std::atomic<float>&>(
            mutable_vcf_options.termination_threshold);
        float new_termination_threshold =
            mutable_vcf_options.termination_threshold.load(
                std::memory_order_relaxed) *
                (1 - mutable_vcf_options.termination_weight) +
            cur_sim * mutable_vcf_options.termination_weight;
...
VersionSet::VersionSet(
    ...
    const std::unordered_map<
        uint32_t, VECTORBACKEND_NAMESPACE::VectorColumnFamilyInfo>* vcf_info,
    port::Mutex* vcf_mutex)
