...
class MemTable {
 public:
  ...
  uint64_t GetLatestTimestamp() {
    return latest_key_time_.load(std::memory_order_relaxed);
  }
  ...
  // Return the type of memtablerep.
  const char* VectorIndexName() const { return table_->VectorIndexName(); }

  // Return the vector index(only hnsw now)
  using dist_t = float;
  std::shared_ptr<hnswlib::HierarchicalNSW<dist_t>> GetVectorIndex() const;

  // Convert private to protected so that HnswMemTable can access it.
 protected:
  ...
  std::atomic<uint64_t> latest_key_time_;
