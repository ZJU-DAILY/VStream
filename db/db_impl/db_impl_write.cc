...
Status DBImpl::WriteImpl(const WriteOptions& write_options,
  ...
  Status status;
  SequenceNumber last_updated_seq;
  ...
  if (status.ok()) {
    ...
    last_updated_seq = last_sequence;
  ...
  std::string msg;
  PutFixed64(&msg, *reinterpret_cast<uint64_t*>(&w.sequence));
  if (auto* state = status.getState(); state != nullptr) {
    msg.append(state);
  }
  status = Status(status.code(), status.subcode(), status.severity(), msg);
  return status;
}
