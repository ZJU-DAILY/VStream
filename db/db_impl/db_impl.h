...
class DBImpl : public DB {
 public:
  ...
  using DB::SetVectorOptions;
  Status SetVectorOptions(
      ColumnFamilyHandle* column_family,
      const std::unordered_map<std::string, std::string>& options_map) override;
  ...
  using DB::GetVectorOptions;
  virtual Status GetVectorOptions(
      ColumnFamilyHandle* column_family,
      VECTORBACKEND_NAMESPACE::VectorOptions* vector_options) const override;
  using DB::Flush;
 ...
 protected:
  ...
  std::unordered_map<uint32_t, VECTORBACKEND_NAMESPACE::VectorColumnFamilyInfo>*
      vcf_info_;
  mutable port::Mutex vcf_mutex_;
