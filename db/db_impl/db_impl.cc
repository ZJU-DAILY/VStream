...
DBImpl::DBImpl(const DBOptions& options, const std::string& dbname,
  ...
  versions_.reset(new VersionSet(
      ...
      io_tracer_, db_id_, db_session_id_, vcf_info_, &vcf_mutex_));
...
Status DBImpl::SetVectorOptions(
    rocksdb::ColumnFamilyHandle* column_family,
    const std::unordered_map<std::string, std::string>& options_map) {
  Status s;
  auto opts = options_map;
  enum kVectorOptionsEnum {
    dim = 0,
    space = 1,
    max_elements = 2,
    M = 3,
    ef_construction = 4,
    random_seed = 5,
    visit_list_pool_size = 6,
    allow_replace_deleted = 7,
    termination_factor = 8,
    termination_threshold = 9,
    termination_weight = 10,
    termination_lower_bound = 11
  };
  std::unordered_map<kVectorOptionsEnum, std::string> vector_opts;
  static const std::string kVectorOptions[]{"dim",
                                            "space",
                                            "max_elements",
                                            "M",
                                            "ef_construction",
  ...
  for (size_t i = 0; i < 10; i++) {
    if (auto it = options_map.find(kVectorOptions[i]);
        it != options_map.end()) {
      vector_opts.insert(
          std::make_pair(static_cast<kVectorOptionsEnum>(i), it->second));
      opts.erase(it);
    }
  }
  if (!opts.empty()) {
    s = SetOptions(column_family, opts);
    if (!s.ok()) {
      return s;
    }
  }
  // Make sure base db's SetOptions success before setting vector options.
  if (!vector_opts.empty()) {
    uint32_t cf_id = column_family->GetID();
    {
      MutexLock l(&vcf_mutex_);
      assert(vcf_info_->count(cf_id) > 0);
      auto& vcf_info = vcf_info_->at(cf_id);
      for (const auto& opt : vector_opts) {
        switch (opt.first) {
          case dim:
            vcf_info.immutable_vcf_options.dim = std::stoi(opt.second);
            break;
          case space:
            if (opt.second == "L2") {
              vcf_info.immutable_vcf_options.space = hnswlib::SpaceType::L2;
            } else if (opt.second == "IP") {
              vcf_info.immutable_vcf_options.space = hnswlib::SpaceType::IP;
            } else {
              return Status::InvalidArgument("unknown space type");
            }
            break;
          case max_elements:
            vcf_info.mutable_vcf_options.max_elements = std::stoi(opt.second);
            break;
          case M:
            vcf_info.mutable_vcf_options.M = std::stoi(opt.second);
            break;
          case ef_construction:
            vcf_info.mutable_vcf_options.ef_construction =
                std::stoi(opt.second);
            break;
          case random_seed:
            vcf_info.mutable_vcf_options.random_seed = std::stoi(opt.second);
            break;
          case visit_list_pool_size:
            vcf_info.mutable_vcf_options.visit_list_pool_size =
                std::stoi(opt.second);
            break;
          case allow_replace_deleted:
            if (opt.second == "true") {
              vcf_info.immutable_vcf_options.allow_replace_deleted = true;
            } else if (opt.second == "false") {
              vcf_info.immutable_vcf_options.allow_replace_deleted = false;
            } else {
              return Status::InvalidArgument(
                  "unknown allow_replace_deleted type");
            }
            break;
          case termination_threshold:
            vcf_info.mutable_vcf_options.termination_threshold =
                std::stof(opt.second);
            break;
          case termination_weight:
            vcf_info.mutable_vcf_options.termination_weight =
                std::stof(opt.second);
            break;
          case termination_lower_bound:
            vcf_info.mutable_vcf_options.termination_lower_bound =
                std::stof(opt.second);
            break;
        }
      }
...
Status DBImpl::CreateColumnFamilies(
    const std::vector<VECTORBACKEND_NAMESPACE::VectorCFDescriptor>& descs,
    std::vector<ColumnFamilyHandle*>* handles) {
  ...
  Status s = CreateColumnFamilies(base_descs, handles);
  ...
  if (s.ok()) {
    ...
    for (size_t i = 0; i < descs.size(); ++i) {
      ...
      auto options = descs[i].options;
      vcf_info_->emplace(
          cf_id, VECTORBACKEND_NAMESPACE::VectorColumnFamilyInfo(
                     handle->GetName(),
                     VECTORBACKEND_NAMESPACE::ImmutableVectorCFOptions(options),
                     VECTORBACKEND_NAMESPACE::MutableVectorCFOptions(options)));
    }
  }
  if (s.ok()) {
    for (auto& desc : descs) {
      ROCKS_LOG_INFO(immutable_db_options_.info_log.get(),
                     "Created vector column family [%s].", desc.name.c_str());
      desc.options.Dump(immutable_db_options_.info_log.get());
    }
  } else {
    ...
    ROCKS_LOG_INFO(immutable_db_options_.info_log,
                   "Failed to vector create column families %s: %s",
...
Status DBImpl::DropColumnFamily(ColumnFamilyHandle* column_family) {
  ...
  if (s.ok()) {
    s = WriteOptionsFile(true /*need_mutex_lock*/,
                         true /*need_enter_write_thread*/);
    MutexLock l(&vcf_mutex_);
    vcf_info_->erase(column_family->GetID());
  }
  return s;
}
...
Status DBImpl::GetVectorOptions(
    ColumnFamilyHandle* column_family,
    VECTORBACKEND_NAMESPACE::VectorOptions* vector_options) const {
  assert(column_family != nullptr);
  vector_options = new VECTORBACKEND_NAMESPACE::VectorOptions();
  Options base_options = GetOptions(column_family);
  *static_cast<DBOptions*>(vector_options) =
      static_cast<DBOptions>(base_options);
  uint32_t cf_id = column_family->GetID();
  {
    MutexLock l(&vcf_mutex_);
    assert(vcf_info_->count(cf_id) > 0);
    const auto& vcf_info = vcf_info_->at(cf_id);
    *static_cast<VECTORBACKEND_NAMESPACE::VectorColumnFamilyOptions*>(
        vector_options) =
        VECTORBACKEND_NAMESPACE::VectorColumnFamilyOptions(
            static_cast<ColumnFamilyOptions>(base_options),
            vcf_info.immutable_vcf_options, vcf_info.mutable_vcf_options);
  }
  return Status::OK();
}
...
Status DB::CreateColumnFamily(
    const VECTORBACKEND_NAMESPACE::VectorColumnFamilyOptions& options,
    const std::string& name, ColumnFamilyHandle** handle) {
  VECTORBACKEND_NAMESPACE::VectorCFDescriptor desc(name, options);
  return CreateColumnFamily(desc, handle);
}

Status DB::CreateColumnFamily(
    const VECTORBACKEND_NAMESPACE::VectorCFDescriptor& desc,
    ColumnFamilyHandle** handle) {
  std::vector<ColumnFamilyHandle*> handles;
...
Status DB::CreateColumnFamilies(
    ...
    std::vector<ColumnFamilyHandle*>* handles) {
  std::vector<VECTORBACKEND_NAMESPACE::VectorCFDescriptor> descs;
  for (auto& name : names) {
    descs.emplace_back(name, options);
  }
